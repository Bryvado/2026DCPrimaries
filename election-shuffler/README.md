...
The demographic explorer can compare two candidates, show precinct details in tooltips, and use
either individual precinct values or a short-distance nearby-precinct average.
Its automatic findings rank raw geographic correlations or estimate the vote
effect of active settings by rerunning the precinct scenario with one factor
removed at a time. These are place-level associations, not inferred voter
transfers.
