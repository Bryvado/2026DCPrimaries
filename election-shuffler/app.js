...
function renderDemographicExplorer(scenario) {
  ...
  const effectiveMode = (requestedMode === "all" && explorerActive) ? "all" : "factor";
  ...
  const slopePrimary = rawSupportSlope(primaryPlot);
  const slopeOverlay = hasCompare ? rawSupportSlope(overlayPlot) : 0;
  ...
  const primaryFactorByPid = new Map(primaryPlot.map((point) => [point.pid, factorVote(point, slopePrimary)]));
  const overlayFactorByPid = hasCompare
  ...
  // estimate for one candidate, or the factor-driven transfer between two.
  ...
  const primaryMarkers = buildScatterMarkers(primaryPlot, "primary", sizeBasisMax, sizeMag, xScale, yScale, factor, overlayName, (point) => markerValue(point, "primary"));
  const overlayMarkers = hasCompare
    ? buildScatterMarkers(overlayPlot, "overlay", sizeBasisMax, sizeMag, xScale, yScale, factor, overlayName, (point) => markerValue(point, "overlay"))
    : "";
...
function buildScatterMarkers(plot, seriesName, basisMax, mag, xScale, yScale, factor, overlayName, valueFn) {
  const MIN = 4;
  const RANGE = 20;
  return plot.map((point, index) => {
    ...
    const side = (MIN + (norm * RANGE)) * mag;
    const fill = value >= 0 ? "#9ad7c8" : "#f2a69a";
    const stroke = value >= 0 ? "#5fae9a" : "#cf7f6a";
    const half = side / 2;
    const titleName = seriesName === "overlay" ? `${escapeHtml(overlayName)}, ` : "";
    const title = `<title>${titleName}Precinct ${escapeHtml(point.pid)}: ${formatFactorValue(factor, point.x)}, ${formatPct(point.y)} support, ${formatSignedVotes(value)} est. votes</title>`;
    if (seriesName === "overlay") {
      ...
      return `<polygon class="scatter-point sized" data-scatter-series="overlay" data-scatter-index="${index}" points="${pts}" fill="${fill}" style="stroke:${stroke}">${title}</polygon>`;
    }
    return `<rect class="scatter-point sized" data-scatter-series="primary" data-scatter-index="${index}" x="${(cx - half).toFixed(2)}" y="${(cy - half).toFixed(2)}" width="${side.toFixed(2)}" height="${side.toFixed(2)}" fill="${fill}" style="stroke:${stroke}">${title}</rect>`;
  }).join("");
}
