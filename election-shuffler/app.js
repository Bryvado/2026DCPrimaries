...
function renderDemographicExplorer(scenario) {
  ...
  const netMode = state.insightFilters.explorerNetMode === "all" ? "all" : "factor";
  const useIsolation = explorerActive && netMode === "factor" && factorActive;
  const factorEffectMap = useIsolation ? precinctFactorEffect(factor) : null;
  state.explorerNetContext = { map: factorEffectMap, active: explorerActive, factor };

  const sizedSeries = sizeBy === "overlay" ? overlayPlot : primaryPlot;
  const sizeBasisMax = sizedSeries.reduce((max, point) => {
    const basis = explorerActive ? Math.abs(explorerNetVote(point, factorEffectMap)) : Math.max(0, point.scenarioVotes);
    return basis > max ? basis : max;
  }, 0);
  const primaryMarkers = buildScatterMarkers(primaryPlot, "primary", sizeBy === "primary", sizeBasisMax, sizeMag, explorerActive, xScale, yScale, factor, overlayName, factorEffectMap);
  const overlayMarkers = overlayPlot.length
    ? buildScatterMarkers(overlayPlot, "overlay", sizeBy === "overlay", sizeBasisMax, sizeMag, explorerActive, xScale, yScale, factor, overlayName, factorEffectMap)
    : "";
  ...
  if (factorModeBtn) factorModeBtn.disabled = !(explorerActive && factorActive);
  if (els.explorerNetMode) {
    const shown = useIsolation ? "factor" : "all";
    els.explorerNetMode.querySelectorAll(".seg-btn").forEach((btn) => btn.classList.toggle("active", btn.dataset.netmode === shown));
  }
  ...
  renderScatterSizeLegend(explorerActive, sizeBasisMax, sizeMag, sizeBy, overlayName, candidateName, useIsolation, factor);
  renderExplorerSideStats(scenario, key, overlayKey, factor, correlationValue, mode, spatialMode, explorerActive, factorEffectMap, useIsolation);
...
function showScatterTooltip(event, point, candidateName, factor, spatialMode) {
  ...
  const change = scenarioState().active
    ...
    : "";
  const isolationContext = state.explorerNetContext;
  let isolatedLine = "";
  if (isolationContext && isolationContext.map) {
    const isolated = isolationContext.map.get(`${point.modelKey}|${point.pid}`);
    if (isolated !== undefined) {
      isolatedLine = `<div><span>From ${escapeHtml(FACTOR_LABELS[isolationContext.factor].toLowerCase())}</span><strong class="${isolated >= 0 ? "delta-pos" : "delta-neg"}">${formatSignedVotes(isolated)} votes</strong></div>`;
    }
  }
...
function explorerNetVote(point, factorEffectMap) {
  if (factorEffectMap) {
    const isolated = factorEffectMap.get(`${point.modelKey}|${point.pid}`);
    if (isolated !== undefined) return isolated;
  }
  return point.scenarioVotes - point.baseVotes;
}

function buildScatterMarkers(plot, seriesName, sized, basisMax, mag, active, xScale, yScale, factor, overlayName, factorEffectMap) {
  const MIN = 4;
  const RANGE = 20;
  return plot.map((point, index) => {
    ...
    let extraClass = "";
    if (sized) {
      const net = explorerNetVote(point, factorEffectMap);
      const basis = active ? Math.abs(net) : Math.max(0, point.scenarioVotes);
    ...
    const netNote = (sized && active) ? `, ${formatSignedVotes(explorerNetVote(point, factorEffectMap))} net votes` : "";
...
function renderScatterSizeLegend(active, basisMax, mag, sizeBy, overlayName, primaryName, useIsolation, factor) {
  if (!els.scatterSizeLegend) return;
  const who = sizeBy === "overlay" ? (overlayName || "Compare") : primaryName;
  if (basisMax <= 0) {
    els.scatterSizeLegend.innerHTML = `<span class="legend-note">No vote movement to size yet.</span>`;
    return;
  }
  const MIN = 4;
  const RANGE = 20;
  ...
  let basisLabel;
  if (!active) basisLabel = "total votes";
  else if (useIsolation) basisLabel = `isolated: ${FACTOR_LABELS[factor]}`;
  else basisLabel = "net votes, all changes";
  const sign = active
    ? `<span class="legend-sign"><i class="pos"></i>adds</span><span class="legend-sign"><i class="neg"></i>removes</span>`
    : "";
  els.scatterSizeLegend.innerHTML = `<span class="legend-lead">${escapeHtml(who)}</span>${swatches}${sign}<span class="legend-note">${escapeHtml(basisLabel)}</span>`;
}

function renderExplorerSideStats(scenario, key, overlayKey, factor, correlationValue, mode, spatialMode, active, factorEffectMap, useIsolation) {
  if (!els.explorerSideStats) return;
  const points = explorerPoints(scenario, key, factor, mode, spatialMode);
  const net = points.reduce((sum, point) => sum + explorerNetVote(point, factorEffectMap), 0);
  const adds = points.filter((point) => explorerNetVote(point, factorEffectMap) > 0.5).length;
  const basisRow = useIsolation ? "Isolated net, total" : "Net votes, total";
  const rows = [
    ["Correlation r", correlationValue.toFixed(2)],
    [basisRow, active ? formatSignedVotes(net) : "\u2014"],
    ["Precincts adding", active ? `${adds} / ${points.length}` : "\u2014"],
    ["Precincts", String(points.length)],
  ];
  const note = useIsolation
    ? `Markers show ${escapeHtml(FACTOR_LABELS[factor].toLowerCase())} isolated from your other changes \u2014 precinct patterns, not individual voters.`
    : "Markers summarize precinct patterns, not individual voters.";
