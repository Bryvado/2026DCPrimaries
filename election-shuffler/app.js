...
function buildScatterMarkers(plot, seriesName, basisMax, mag, xScale, yScale, factor, overlayName, valueFn, neutral) {
  ...
  return plot.map((point, index) => {
    ...
    if (seriesName === "overlay") {
      ...
      return `<polygon class="scatter-point${neutral ? " overlay-point" : " sized"}" data-scatter-series="overlay" data-scatter-index="${index}" points="${pts}" fill="${fill}" style="stroke:${stroke}">${title}</polygon>`;
    }
    return `<rect class="scatter-point${neutral ? "" : " sized"}" data-scatter-series="primary" data-scatter-index="${index}" x="${(cx - half).toFixed(2)}" y="${(cy - half).toFixed(2)}" width="${side.toFixed(2)}" height="${side.toFixed(2)}" fill="${fill}" style="stroke:${stroke}">${title}</rect>`;
  }).join("");
}
