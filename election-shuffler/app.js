...
async function init() {
  ...
  const [precincts, candidates, terms, predictions, priorMatches, candidateTerritory, officialContestTotals, geojson] = await Promise.all([
    ...
    loadJson("data/precincts.geojson"),
  ]);
...
function renderDemographicExplorer(scenario) {
  ...
  const factorEffects = precinctFactorVoteEffects(factor, scenario);
  for (const point of primaryPlot) {
    point.factorVoteEffect = factorEffects.get(`${key}|${point.pid}`) || 0;
  }
  for (const point of overlayPlot) {
    point.factorVoteEffect = factorEffects.get(`${overlayKey}|${point.pid}`) || 0;
  }
  const factorActive = Math.abs(num(state.demoSliders[factor])) > 0.0001
    || Math.abs(num(state.turnoutSliders[factor])) > 0.0001;
  let sizeBy = (state.insightFilters.explorerSizeBy === "overlay" && overlayKey) ? "overlay" : "primary";
  state.insightFilters.explorerSizeBy = sizeBy;
  const sizeMag = num(state.insightFilters.explorerSizeMag) || 1.2;
  const sizedSeries = sizeBy === "overlay" ? overlayPlot : primaryPlot;
  const sizeBasisMax = sizedSeries.reduce((max, point) => {
    const basis = Math.abs(point.factorVoteEffect || 0);
    return basis > max ? basis : max;
  }, 0);
  const primaryMarkers = buildScatterMarkers(primaryPlot, "primary", sizeBy === "primary", sizeBasisMax, sizeMag, factorActive, xScale, yScale, factor, overlayName);
  const overlayMarkers = overlayPlot.length
    ? buildScatterMarkers(overlayPlot, "overlay", sizeBy === "overlay", sizeBasisMax, sizeMag, factorActive, xScale, yScale, factor, overlayName)
    : "";
  const overlaySegBtn = els.explorerSizeBy ? els.explorerSizeBy.querySelector('[data-sizeby="overlay"]') : null;
  if (overlaySegBtn) overlaySegBtn.disabled = !overlayKey;
  if (els.explorerSizeBy) {
    els.explorerSizeBy.querySelectorAll(".seg-btn").forEach((btn) => btn.classList.toggle("active", btn.dataset.sizeby === sizeBy));
  }
  ...
  renderScatterSizeLegend(factorActive, sizeBasisMax, sizeMag, sizeBy, overlayName, candidateName);
  renderExplorerSideStats(scenario, key, overlayKey, factor, correlationValue, mode, spatialMode, factorActive, factorEffects);
...
function explorerPoints(scenario, modelKey, factor, mode, spatialMode) {
  ...
  const points = scenario.rows
    ...
    .map((row) => {
      ...
      return {
        pid: row.pid,
        x: num(precinct?.[column]),
        y: mode === "base" ? row.base_share : row.scenario_share,
        rawX: num(precinct?.[column]),
...
function showScatterTooltip(event, point, candidateName, factor, spatialMode) {
  ...
  const factorVoteDelta = explorerNetVote(point);
  const factorActive = Math.abs(num(state.demoSliders[factor])) > 0.0001
    || Math.abs(num(state.turnoutSliders[factor])) > 0.0001;
  ...
  const actualValue = spatialMode === "nearby"
    ...
    : `<div><span>${escapeHtml(FACTOR_LABELS[factor])}</span><strong>${formatFactorValue(factor, point.rawX)}</strong></div><div><span>Candidate support</span><strong>${formatPct(point.rawY)}</strong></div>`;
  const factorChange = factorActive
    ? `<div><span>${escapeHtml(FACTOR_LABELS[factor])} vote effect</span><strong class="${factorVoteDelta >= 0 ? "delta-pos" : "delta-neg"}">${formatSignedVotes(factorVoteDelta)} votes</strong></div>`
    : "";
  const totalChange = scenarioState().active
    ? `<div><span>Total scenario vote change</span><strong class="${voteDelta >= 0 ? "delta-pos" : "delta-neg"}">${formatSignedVotes(voteDelta)} votes</strong></div>`
    : "";
  ...
  els.scatterTooltip.innerHTML = `
    ...
    ${nearbyNote}${actualValue}${factorChange}${totalChange}
...
function explorerNetVote(point) {
  return num(point.factorVoteEffect);
}

function buildScatterMarkers(plot, seriesName, sized, basisMax, mag, active, xScale, yScale, factor, overlayName) {
  const MIN = 4;
  const RANGE = 20;
  return plot.map((point, index) => {
    ...
    let extraClass = "";
    if (sized) {
      const net = explorerNetVote(point);
      const basis = Math.abs(net);
      const norm = basisMax > 0 ? Math.sqrt(basis / basisMax) : 0;
      side = (MIN + (norm * RANGE)) * mag;
      fill = active ? (net >= 0 ? "#9ad7c8" : "#f2a69a") : point.color;
      stroke = active ? (net >= 0 ? "#5fae9a" : "#cf7f6a") : "#ffffff";
      extraClass = " sized";
    } else {
      side = 6;
      fill = seriesName === "overlay" ? "#ffffff" : point.color;
      stroke = point.color;
      extraClass = seriesName === "overlay" ? " overlay-point" : "";
    }
    const half = side / 2;
    const titleName = seriesName === "overlay" ? `${escapeHtml(overlayName)}, ` : "";
    const netNote = (sized && active) ? `, ${formatSignedVotes(explorerNetVote(point))} net votes` : "";
    const title = `<title>${titleName}Precinct ${escapeHtml(point.pid)}: ${formatFactorValue(factor, point.x)}, ${formatPct(point.y)} support${netNote}</title>`;
    if (seriesName === "overlay") {
      ...
      return `<polygon class="scatter-point${extraClass}" data-scatter-series="overlay" data-scatter-index="${index}" points="${pts}" fill="${fill}" style="stroke:${stroke}">${title}</polygon>`;
    }
    return `<rect class="scatter-point${extraClass}" data-scatter-series="primary" data-scatter-index="${index}" x="${(cx - half).toFixed(2)}" y="${(cy - half).toFixed(2)}" width="${side.toFixed(2)}" height="${side.toFixed(2)}" fill="${fill}" style="stroke:${stroke}">${title}</rect>`;
  }).join("");
}

function renderScatterSizeLegend(active, basisMax, mag, sizeBy, overlayName, primaryName) {
  if (!els.scatterSizeLegend) return;
  const who = sizeBy === "overlay" ? (overlayName || "Compare") : primaryName;
  if (basisMax <= 0) {
    els.scatterSizeLegend.innerHTML = `<span class="legend-note">The selected factor has no active vote effect.</span>`;
    return;
  }
  const MIN = 4;
  const RANGE = 20;
  const sideFor = (value) => (MIN + ((basisMax > 0 ? Math.sqrt(value / basisMax) : 0) * RANGE)) * mag;
  const steps = [0.18, 0.5, 1]
  ...
  const sign = active
    ? `<span class="legend-sign"><i class="pos"></i>adds</span><span class="legend-sign"><i class="neg"></i>removes</span>`
    : `<span class="legend-note">no active shift</span>`;
  els.scatterSizeLegend.innerHTML = `<span class="legend-lead">${escapeHtml(who)}</span>${swatches}${sign}`;
}

function renderExplorerSideStats(scenario, key, overlayKey, factor, correlationValue, mode, spatialMode, active, factorEffects) {
  if (!els.explorerSideStats) return;
  const points = explorerPoints(scenario, key, factor, mode, spatialMode);
  const effects = points.map((point) => factorEffects.get(`${key}|${point.pid}`) || 0);
  const net = effects.reduce((sum, value) => sum + value, 0);
  const adds = effects.filter((value) => value > 0.5).length;
  const rows = [
    ["Correlation r", correlationValue.toFixed(2)],
    ["Selected-factor votes", active ? formatSignedVotes(net) : "\u2014"],
    ["Precincts adding", active ? `${adds} / ${points.length}` : "\u2014"],
    ["Precincts", String(points.length)],
  ];
  els.explorerSideStats.innerHTML = `<span class="eside-title">${escapeHtml(FACTOR_LABELS[factor])}</span>`
    + rows.map(([label, value]) => `<div class="eside-row"><span>${label}</span><b>${value}</b></div>`).join("")
    + `<p class="eside-note">Markers summarize precinct patterns, not individual voters.</p>`;
}
...
function precinctFactorVoteEffects(factor, currentScenario) {
  const supportValue = state.demoSliders[factor] || 0;
  const turnoutValue = state.turnoutSliders[factor] || 0;
  const effects = new Map();
  if (supportValue === 0 && turnoutValue === 0) return effects;

  let withoutFactor;
  try {
    state.demoSliders[factor] = 0;
    state.turnoutSliders[factor] = 0;
    withoutFactor = runScenario(state.contestNumber);
  } finally {
    state.demoSliders[factor] = supportValue;
    state.turnoutSliders[factor] = turnoutValue;
  }

  const withoutByKey = new Map(
    withoutFactor.rows.map((row) => [`${row.model_key}|${row.pid}`, row.scenario_votes])
  );
  for (const row of currentScenario.rows) {
    const key = `${row.model_key}|${row.pid}`;
    effects.set(key, row.scenario_votes - num(withoutByKey.get(key)));
  }
  return effects;
}
...
function demographicCaption(candidateName, factorLabel, value, mode, overlayName = "", overlayValue = null, spatialMode = "raw", scaleMode = "shared") {
  ...
  const overlay = overlayName && overlayValue !== null
    ...
    : "";
  const geography = spatialMode === "nearby"
    ? " Values use a short-distance average of each precinct and its nearest neighbors."
    : "";
  const scale = scaleMode === "log"
    ? " The vertical scale is logarithmic so candidates with different support levels remain visible."
    : scaleMode === "relative"
      ? " Each candidate is scaled from their weakest to strongest precinct; vertical positions are not vote percentages."
      : "";
  const view = scenarioState().supportActive && mode === "scenario" ? " This chart includes your changes." : "";
  return `${relationship} (r = ${value.toFixed(2)}).${overlay}${geography}${scale} The lines summarize precinct patterns; they do not describe individual voters.${view}`;
}
  ...
  state.precinctGeoLayer = L.geoJSON(geojson, {
    ...
    onEachFeature: (feature, layer) => {
      ...
      layer.on("mouseover", () => {
        ...
        layer.setStyle({ color: "#ffffff", opacity: 1, weight: 2 });
        layer.bringToFront();
      });
      layer.on("mouseout", () => layer.setStyle(mapStyleForPid(pid)));
    },
  }).addTo(state.leafletMap);
...
function paintMap() {
  ...
  for (const [pid, layer] of state.precinctLayers.entries()) {
    layer.setStyle(mapStyleForPid(pid));
    layer.setTooltipContent(mapTooltipHtml(pid));
  }
}
...
function mapStyleForPid(pid) {
  ...
  if (state.mapMode === "winner") {
    style.fillOpacity = clamp(0.42 + (winner.scenario_share / 100) * 0.34, 0.42, 0.76);
  } else if (state.mapMode === "margin") {
    const margin = winner.scenario_share - (second?.scenario_share || 0);
    style.fillOpacity = 0.25 + clamp(margin / 60, 0, 1) * 0.53;
  } else {
...
async function fetchAsset(path) {
  ...
  for (const candidate of candidates) {
    ...
    if (response.ok) {
      if (candidate !== path) console.info(`Loaded ${path} from ${candidate}`);
      return response;
    }
  }
  throw new Error(`Failed to load ${path}. Upload the data folder beside index.html or one directory above it.`);
}
