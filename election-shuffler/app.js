...
const els = {
  ...
  showPrecinctLabels: document.querySelector("#showPrecinctLabels"),
...
function bindInsightEvents() {
  ...
  els.showPrecinctLabels.addEventListener("change", () => {
    state.insightFilters.showPrecinctLabels = els.showPrecinctLabels.checked;
    renderDemographicExplorer(state.latestScenario);
  });
...
function syncInsightControls(summary) {
  ...
  els.explorerOverlay.innerHTML = `<option value="">No overlay</option>` + candidates
  ...
  els.showPrecinctLabels.checked = filters.showPrecinctLabels;
...
function renderDemographicExplorer(scenario) {
  ...
  const primaryXs = points.map((point) => point.x);
  const xs = primaryXs.concat(overlayPoints.map((point) => point.x));
  const primaryYs = points.map((point) => point.y);
  const ys = primaryYs.concat(overlayPoints.map((point) => point.y));
  ...
  const overlayFit = overlayPoints.length ? linearRegression(overlayPoints) : null;
  ...
  els.scatterPlot.innerHTML = `
    ...
    ${overlayPoints.map((point) => `
      <circle class="scatter-point overlay-point" cx="${xScale(point.x).toFixed(2)}" cy="${yScale(point.y).toFixed(2)}" r="4" fill="#fff" style="stroke:${point.color}">
    ...
    `).join("")}
    ${state.insightFilters.showPrecinctLabels ? points.map((point) => `
      <text class="precinct-point-label" x="${(xScale(point.x) + 5).toFixed(2)}" y="${(yScale(point.y) - 5).toFixed(2)}">${escapeHtml(point.pid)}</text>
    `).join("") : ""}
  `;
}
...
function demographicCaption(candidateName, factorLabel, value, mode, overlayName = "", overlayValue = null, spatialMode = "raw") {
  ...
  return `${relationship} (r = ${value.toFixed(2)}).${overlay}${geography} The lines summarize precinct patterns; they do not describe individual voters.${view}`;
}
