...
function renderDemographicExplorer(scenario) {
  ...
  els.scatterPlot.innerHTML = `
    ...
    ${primaryPlot.map((point, index) => `
      <circle class="scatter-point" data-scatter-series="primary" data-scatter-index="${index}" cx="${xScale(point.x).toFixed(2)}" cy="${yScale(point.plotY).toFixed(2)}" r="4" fill="${point.color}">
        <title>Precinct ${escapeHtml(point.pid)}: ${formatFactorValue(factor, point.x)}, ${formatPct(point.y)} support</title>
      </circle>
    `).join("")}
    ${overlayPlot.map((point, index) => `
      <circle class="scatter-point overlay-point" data-scatter-series="overlay" data-scatter-index="${index}" cx="${xScale(point.x).toFixed(2)}" cy="${yScale(point.plotY).toFixed(2)}" r="4" fill="#fff" style="stroke:${point.color}">
        <title>${escapeHtml(overlayName)}, Precinct ${escapeHtml(point.pid)}: ${formatFactorValue(factor, point.x)}, ${formatPct(point.y)} support</title>
      </circle>
    `).join("")}
  `;
